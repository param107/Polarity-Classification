        ...
        for i in self.tweets:
            ...
            for t in tw:
                ...
                if(label == 'positive'):
                    self.pos_count[i] += 1
                    posc = 0 
                    for a in range(len(self.pos_count)):
                        posc = posc + self.pos_count[a]


                elif(label == 'negative'):                
                    self.neg_count[i] += 1
                    negc = 0 
                    for j in range(len(self.neg_count)):
                        negc = negc + self.neg_count[j]

                elif(label == 'neutral'):                
                    self.neut_count[i] += 1
                    neuc = 0 
                    for k in range(len(self.neut_count)):
                        neuc = neuc + self.neut_count[k]
            ...
            #end inner loop
            self.results[i] = res
        print "Polarity Classification Using Maximum Entropy Classifier"
        total = posc+negc+neuc
        print "Total tweets found", total
        print "Positive Polarity", posc*100/total, "%"
        print "Negative Polarity", negc*100/total, "%"
        print "Neutral Polarity", neuc*100/total, "%"
